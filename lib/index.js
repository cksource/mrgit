...
const createForkPool = require( './utils/create-fork-pool' );
const logDisplay = require( './utils/log-display' );
	...
	function onDone() {
		return forkPool.killAll()
			.then( () => {
				...
				console.log( chalk.cyan( `Execution time: ${ endTime[ 0 ] }s ${ endTime[ 1 ].toString().substring( 0, 5 ) }ns.` ) );
			} );
	}
};
