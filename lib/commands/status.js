...
module.exports = {
	...
	execute( data ) {
		...
		const promises = [
			...
			execCommand.execute( getExecData( 'git tag --sort=committerdate | grep -E \'[0-9]\' --color=none | tail -1' ) )
		];
		...
		return Promise.all( promises )
			.then( ( [ hashResponse, currentBranchStatusResponse, currentTagStatusResponse, latestTagStatusResponse ] ) => {
				...
				const currentBranch = currentBranchStatusResponse.logs.info[ 0 ];
				const currentTag = currentTagStatusResponse.logs.info[ 0 ];
				const latestTag = latestTagStatusResponse.logs.info[ 0 ];
				...
				const commandResponse = {
					packageName,
					status: gitStatusParser( currentBranch, currentTag ),
	...
	afterExecute( processedPackages, commandResponses ) {
		...
		function createSingleRow( data ) {
			const { packageName, status, commit, mrgitBranch, mrgitTag, currentTag, latestTag } = data;
			const wholeRow = [];
			const statusColumn = [];

			const wholeRowColor = status.branchOrTag !== 'master' ? 'magenta' : null;
			const shouldUseLatest = mrgitTag === 'latest';
			let branchOrTag = status.branchOrTag;
			...
			if ( mrgitTag && shouldUseLatest && status.branchOrTag === currentTag && currentTag === latestTag ) {
				branchOrTag = `${ color( 'green', 'L' ) } ${ branchOrTag }`;
			}

			if ( mrgitTag && shouldUseLatest && status.branchOrTag === currentTag && currentTag !== latestTag ) {
				branchOrTag = `${ color( 'yellow', 'L' ) } ${ branchOrTag }`;
			}

			if ( mrgitTag && status.branchOrTag !== currentTag ) {
				branchOrTag = `${ color( 'cyan', '!' ) } ${ branchOrTag }`;
			}

			if ( mrgitTag && !shouldUseLatest && status.branchOrTag !== mrgitTag ) {
				branchOrTag = `${ color( 'cyan', '!' ) } ${ branchOrTag }`;
			}

			if ( !mrgitTag && status.branchOrTag !== mrgitBranch ) {
				branchOrTag = `${ color( 'cyan', '!' ) } ${ branchOrTag }`;
			}

			if ( status.ahead ) {
				branchOrTag += color( 'yellow', ` ↑${ status.ahead }` );
			}

			if ( status.behind ) {
				branchOrTag += color( 'yellow', ` ↓${ status.behind }` );
			}
			...
			wholeRow.push( color( wholeRowColor, branchOrTag ) );
		...
		function getLegend() {
			const legend = [
				...
				`\n${ color( 'cyan', '!' ) } not on a branch or a tag specified in "mrgit.json"`,
				`\n${ color( 'green', 'L' ) } up to date "latest" tag`,
				`${ color( 'yellow', 'L' ) } newer "latest" tag is available`
			];
