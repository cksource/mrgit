		...
		return execCommand.execute( getExecData( 'git status -s' ) )
			...
			.then( async () => {
				...
				if ( !data.repository.tag ) {
					...
					const command = 'git tag --sort=committerdate | grep -E \'[0-9]\' --color=none | tail -1';
					const commandOutput = await execCommand.execute( getExecData( command ) );
					const latestTag = commandOutput.logs.info[ 0 ].trim();
		...
		return shell( `git clone --progress "${ packageDetails.url }" "${ packageDetails.path }"` )
			.then( async output => {
				...
				if ( !packageDetails.tag ) {
					...
					const latestTag = await shell( `cd "${ packageDetails.path }" && git describe --abbrev=0 --tags` );
					checkoutValue = 'tags/' + latestTag.trim();
				} else {
