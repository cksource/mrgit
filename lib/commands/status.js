...
module.exports = {
	...
	afterExecute( processedPackages, commandResponses ) {
		...
		let shoutDisplayLatestHint = false;
		let shoutDisplaySyncHint = false;
		...
		displayLegend();
		displayHints( shoutDisplayLatestHint, shoutDisplaySyncHint );
		...
		function createSingleRow( data ) {
			...
			if ( shouldUseTag && shouldUseLatestTag && status.detachedHead && status.tag === latestTag ) {
				...
				shoutDisplayLatestHint = true;
			}

			if ( shouldUseTag && shouldUseLatestTag && ( !status.detachedHead || status.tag !== latestTag ) ) {
				branchOrTag = `${ chalk.cyan( '!' ) } ${ branchOrTag }`;
				shoutDisplaySyncHint = true;
			}
		...
		function displayHints( shoutDisplayLatestHint, shoutDisplaySyncHint ) {
			const hints = [];

			if ( shoutDisplayLatestHint ) {
				hints.push( [
					chalk.green( 'L' ),
					'This is the latest local tag. To ensure having latest origin tag, execute',
					chalk.blue( 'mrgit sync' )
				].join( ' ' ) + '.' );
			}

			if ( shoutDisplaySyncHint ) {
				hints.push( [
					chalk.cyan( '!' ),
