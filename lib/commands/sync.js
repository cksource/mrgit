...
export default {
	 ...
	 */
	async afterExecute( parsedPackages, commandResponses, toolOptions ) {
		...
		return shell( `git clone --progress "${ packageDetails.url }" "${ packageDetails.path }"` )
			...
			.then( async () => {
				...
				if ( toolOptions.recursive ) {
					const packageJsonPath = upath.join( packageDetails.path, 'package.json' );
					const { default: packageJson } = await import( pathToFileURL( packageJsonPath ).href, { with: { type: 'json' } } );
					const packages = [];
