...
module.exports = {
	...
	_clonePackage( packageDetails, toolOptions, options ) {
		...
		if ( !packageDetails.tag ) {
			command.push( `git checkout --quiet ${ packageDetails.branch }` );
		} else if ( packageDetails.tag === 'latest' ) {
			command.push( 'LATEST=$( git describe --abbrev=0 --tags )', 'git checkout --quiet $LATEST' );
		} else {
			command.push( `git checkout --quiet ${ packageDetails.tag }` );
		}
