...
export default {
	...
	beforeExecute( _1, _2, task ) {
		task.output = chalk.blue( 'Collecting statuses...' );
	},
	...
	async execute( data ) {
		...
		const git = simpleGit( {
			baseDir: data.repositoryPath
		} );

		const { latestTag, currentTag } = await this._getTagInfo( git );
		const hashResponse = await git.raw( 'rev-parse', '--short', 'HEAD' );
		const currentBranchStatusResponse = await git.status();
		...
		const commandResponse = {
			...
			status: gitStatusParser( currentBranchStatusResponse, currentTag ),
			commit: hashResponse.trim(),
	...
	async _getTagInfo( git ) {
		const latestTagStatusResponse = await git.raw( [
			'log',
			'--tags',
		...
		if ( latestTagStatusResponse.length ) {
			const currentTagStatusResponse = await git.raw( [
				'describe',
				'--abbrev=0',
				'--tags',
				'--always'
			...
			latestTag = latestTagStatusResponse.trim().split( '\n' ).shift();
			currentTag = currentTagStatusResponse.trim();
		}
