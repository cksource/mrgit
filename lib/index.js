 ...
 * @param {String} options.repositoryResolver Module which will resolve repositories for packages.
 */
module.exports = function( parameters, options ) {
	const repositoryResolver = require( options.repositoryResolver );
	...
	function enqueue( packageName ) {
		...
		const data = {
			...
			options,
			name: packageName,
			repository: repositoryResolver( packageName, options.cwd ),
			mgit: mgitConf
		};
