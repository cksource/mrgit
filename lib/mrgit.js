...
function mapPackageToListrTask( packageName, rootPackageName ) {
	...
	return {
		...
		task: async ( ctx, task ) => {
			const repository = ctx.repositoryResolver( packageName, ctx.toolOptions, isRootRepository );

			if ( !repository ) {
				throw new Error( 'Cannot resolve a repository' );
			}
			...
			const commandData = {
				...
				repositoryPath: isRootRepository ?
					ctx.toolOptions.cwd :
					upath.join( ctx.toolOptions.packages, repository.directory )
			};
			...
			const subPackages = ( Array.isArray( returnedData.packages ) ? returnedData.packages : [] )
				...
				.filter( item => !ctx.processedPackages.has( item ) )
