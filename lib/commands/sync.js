		...
		return execCommand.execute( getExecData( 'git status -s' ) )
			...
			.then( async () => {
				...
				if ( !data.repository.tag ) {
					...
					const commandOutput = await execCommand.execute( getExecData( 'git tag --sort=committerdate' ) );
		...
		return shell( `git clone --progress "${ packageDetails.url }" "${ packageDetails.path }"` )
			.then( async output => {
				...
				if ( !packageDetails.tag ) {
					...
					const commandOutput = await shell( `cd "${ packageDetails.path }" && git tag --sort=committerdate` );
